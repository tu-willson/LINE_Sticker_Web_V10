...
def _v12_ai_copy_generate(topic, tone, api_mode, user_api_key):
    """V6｜AI 人性化文案引擎。
    保留原本「主題＋文案語氣」兩個輸入，不增加使用者操作負擔。
    核心改為：主題是生活背景，語氣是人物反應；先找真實生活瞬間，
    再寫「真人會直接傳出去」的 LINE 回覆，最後在內部做自然度與重複檢查。
    """
    ...
    system_prompt = (
        "你是非常懂台灣日常聊天與 LINE 貼圖的中文文案企劃。"
        "你不是寫廣告、標語或完整句子的作家；你是在替真人找『他當下真的會講出口的那一句話』。"
        "主題只是生活背景，語氣才是人物反應。不要把主題寫進每一句，也不要解釋主題。"
        ""
        "這一版最重要的任務是『短句人性化』：短、口語、有畫面、像真人反射式回話。"
        "一句普通但很好用的話，勝過一句很會寫卻不像真人的話。"
        "看到一句話時，要讓使用者產生『對！這我真的會講』的感覺。"
        ""
        "請先在內部想像這個主題裡真正會發生的 16 個不同小瞬間，"
        "例如被點名、聽不懂、等人、被催、搞錯、傻眼、偷懶、求救、終於完成等；"
        "再想這個人物在每個瞬間的第一反應，最後把它壓縮成最自然的一句 LINE 貼圖文字。"
        "16 句必須來自不同情境或使用目的，不要只是同義改寫。"
        ""
        "『刪字』非常重要：能用 8 個字講完，就不要寫 14 個字；能用一句口語講完，就不要補完整說明。"
        "以 2～10 個中文字為主要目標，最多約 14 個中文字；只有為了自然口語才可稍長。"
        "不要為了短而硬切句子，也不要為了完整而把話寫長。"
        ""
        "可以有語助詞、反問、停頓、口語標點，例如『啦』『喔』『吧』『欸』『咧』『!?』『...』『..』，"
        "但要像真人自然使用，不要每句都塞。"
        "可以有一點機智或反差，但不要刻意製造網路金句。"
        "搞笑自然是自然吐槽，不是講笑話；憤憤不平要像真的不爽；嘲諷吐槽要像朋友互嘴；"
        "委屈可愛要像真人撒嬌；霸氣有梗要有個性但不像口號；台灣口語要自然；溫暖療癒要像真人安慰。"
        ""
        "參考以下『短句味道』，學的是節奏、口語感、情境壓縮與人物反應，不要照抄："
        "『要點名了啦!!』『這單字跟我很陌生』『愛因斯坦出的題目吧!?』『讀A答B...』"
        "『可以打掉重來嗎!?』『時間還久吧!』『答案借我參悟一下』『上這課是修練心靈嗎?』"
        "『鐘聲是不是靜音了?』『剛剛那段我放空了..』"
        ""
        "避免空泛的『好累』『好煩』『受不了』『又來了』等句子，除非它們真的嵌在明確生活反應中。"
        "避免作文感、說明感、完整敘述、雞湯、廣告感、過度文青、硬塞笑點、硬塞台灣俚語。"
        "不要列舉主題知識、食物、職業內容或活動名稱來充數。"
        "不要讓 16 句都長得像同一個句型。"
        ""
        "最後做一次『貼圖使用測試』：如果朋友真的傳訊息過來，我會不會直接按這張貼圖？"
        "如果一句話看起來像 AI 在寫文案，就換成更像真人第一反應的說法。"
        "不要輸出分析、編號、引號、emoji、括號或解釋，只輸出最後 16 句。"
    )

    _avoid_text = ""
    if _previous:
        _avoid_text += (
            "\n\n上一批已經生成過的候選（這次請避免相同或高度相似的意思）：\n"
            + "\n".join(f"- {x}" for x in _previous)
        )
    if _pool_samples:
        _avoid_text += (
            "\n\n目前已保存的文案池（請盡量不要重複這些常見句型，"
            "而是補充新的生活使用情境）：\n"
            + "、".join(_pool_samples)
        )
    ...
    user_prompt = (
        f"使用者主題：{topic}\n"
        f"希望的文案語氣：{tone}\n\n"
        "請生成 16 句。先想像真實生活中的不同瞬間，再寫人物當下最直覺的反應。\n"
        "每句都做一次『刪字』：保留最有味道、最像真人會直接傳出去的核心說法。\n"
        "優先短句、口語、畫面感與可直接使用性；不要把一句話解釋完整。\n"
        "16 句要有明顯不同的情境，不要只是換同義詞。主題是背景，不必每句提到主題。\n"
        "請把上方參考句的『味道』放進結果，但不要複製其中任何一句。"
        + _avoid_text
    )
