...
def _v12_ai_copy_generate(topic, tone, api_mode, user_api_key):
    ...
    system_prompt = (
        ...
        "【內部候選與角度競爭】"
        "請在內部先針對不同生活畫面產生比最後 16 句更多的候選，並讓不同角度互相競爭。"
        "同一個畫面若只有一種普通說法，先不要交卷；再嘗試一次反差、比喻、擬人、諧音、經典梗或台灣口語。"
        "最後淘汰：作文感、廣告感、雞湯感、過度文青、只是同義改寫、只有漂亮沒有畫面、只有梗沒有情境、明顯 AI 句型。"
        "同時淘汰太多『我好累／好煩／受不了／要完蛋』這類沒有角度的泛句。"
        "但是如果某個很普通的句子真的非常好用，不要為了炫技而刪掉它。\n\n"
        "【真人使用測試】"
        "最後每句都做內部測試：看到文字能否立刻想像一個畫面？像不像台灣人真的會說？如果朋友傳來，我會不會真的想按這張貼圖？"
    ...
    user_prompt = (
        f"使用者主題：{topic}\n"
        f"希望的文案語氣：{tone}\n\n"
        "請開始一次新的 16 句創作。先在內部建立多個可視化的真實生活瞬間，"
        "再為每個瞬間尋找不同的『好玩角度』，例如反差、擬人、比喻、諧音、雙關、經典梗、台灣口語或意外轉換，"
        "然後想像真人第一反應，最後刪到短而有味道。"
        "請讓結果像真正可以拿來回 LINE 的話，而不是像文案作業。"
        "經典老梗可以直接用，普通但好用也可以，偶爾允許一兩句讓人意外的腦洞。"
        + _avoid_text
    )
