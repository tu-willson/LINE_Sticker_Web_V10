...
def _v12_ai_copy_generate(topic, tone, api_mode, user_api_key):
    ...
    system_prompt = (
        ...
        "【內部候選與角度競爭｜V6.4】"
        "請在內部先針對不同生活畫面產生比最後 16 句更多的候選，並讓不同角度互相競爭。至少讓候選池中同時存在：真正口語的句子、經典／老梗、具體情境句、比喻／擬人、語意反轉、台灣語感、諧音／雙關、少量意外腦洞。不要平均配額；只要某一類在該主題特別適合，就可以多一些。"
        "同一個畫面若只有一種普通說法，可以再嘗試 1～2 個不同角度，但不要為了變化硬湊。特別尋找『換一個角度就突然有味道』的說法。"
        "最後淘汰：作文感、廣告感、雞湯感、過度文青、只是同義改寫、只有漂亮沒有畫面、只有梗沒有情境、明顯 AI 句型。"
        "同時淘汰太多『我好累／好煩／受不了／要完蛋』這類沒有具體畫面的泛句；除非它本身就是極其自然、極其好用的真人口語。"
        "但是如果某個很普通的句子真的非常好用，不要為了炫技而刪掉它。\n\n"
        "【真人使用測試】"
    ...
    user_prompt = (
        f"使用者主題：{topic}\n"
        f"希望的文案語氣：{tone}\n\n"
        "請開始一次新的 16 句創作。先在內部建立多個具體、可視化的真實生活小瞬間，"
        "再為每個瞬間尋找不同的『好玩角度』，包括真人原話、反差、擬人、比喻、諧音、雙關、經典老梗、台灣口語或意外轉換；不要為了創意而犧牲自然。"
        "然後想像真人第一反應，嘗試一兩個不同角度，再刪到短而有味道。"
        "請讓結果像真正可以拿來回 LINE 的話，而不是像文案作業。"
        "經典老梗可以直接用，普通但好用也可以；同一批裡可以出現一些經典、一​​些生活口語、一些情境梗，再留少量不可預測的腦洞。不要刻意平均分配。"
        + _avoid_text
    )
